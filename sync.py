...
class Bc(WrappedCommand):
    ...
    def _invoke(self, arg, from_tty):
        ...
        if arg.find('color') != -1:
            arg_list = arg.split(' ')
            arg ='set'
            if(len(arg_list) == 2 and arg_list[1]) == '2':
                rgb = 0xffff00 # blue
            else:
                rgb = 0x33ff00 #green
            self.sync.tunnel.send("[notice]{\"type\":\"bc\",\"msg\":\"%s\",\"rgb\":%d,\"base\":%d,\"offset\":%d}\n" %
                            (arg,rgb, self.sync.base, self.sync.offset))    
            return 
            
        if arg.find('set') != -1:
            arg_list = arg.split(' ')
            arg = arg_list[0]
            rgb = int(arg_list[1],0)
            self.sync.tunnel.send("[notice]{\"type\":\"bc\",\"msg\":\"%s\",\"rgb\":%d,\"base\":%d,\"offset\":%d}\n" %
                            (arg,rgb, self.sync.base, self.sync.offset))
            return 


        if not (arg in ["on", "off", "oneshot", "set"]):
            rs_log("usage: bc <|on|off|set color_id>")
            return
